...
public class CJavaMHIBasel3IssuerRatingDerivation implements JavaRule {
    private final static Logger logger = Logger.getLogger("CJavaMHIBasel3IssuerRatingDerivation");
    ...
    private String deriveRatingQuery = " SELECT BRTVL.RTNG_CDE, VRTNG.DATA_SRC_ID, BRM1.BRM1_OID, BRTVL.RTNG_VALUE_OID, VRTNG.RTNG_CQS_NUM " +
        " FROM FT_T_FIRT FIRT, FT_T_BRM1 BRM1, " +
        ...
        "	AND RTG.END_TMS IS NULL AND RTV.END_TMS IS NULL AND BC.END_TMS IS NULL) VRTNG, FT_T_RTVL BRTVL WHERE FIRT.RTNG_SET_OID = BRM1.VND_RTNG_SET_OID " +
        " AND (FIRT.RTNG_CURR_TYP = BRM1.VND_RTNG_CURR_TYP OR (FIRT.RTNG_CURR_TYP IS NULL AND BRM1.VND_RTNG_CURR_TYP IS NULL) ) " +
        " AND (FIRT.RTNG_DEBT_CL_TYP = BRM1.MDY_RTNG_DEBT_CL_TYP OR BRM1.MDY_RTNG_DEBT_CL_TYP IS NULL) AND FIRT.RTNG_SET_OID = VRTNG.RTNG_SET_OID AND VRTNG.RTNG_CDE = FIRT.RTNG_CDE " +
        ...
        " AND (BRM1.END_TMS IS NULL OR BRM1.END_TMS > SYSDATE()) AND BRM1.RTNG_SET_TBL_TYP IN ('FINS','FIIS','ALL')and (FIRT.END_TMS IS NULL OR FIRT.END_TMS > SYSDATE()) " +
        " AND (FIRT.SYS_EFF_END_TMS  IS NULL OR FIRT.SYS_EFF_END_TMS > SYSDATE()) AND FIRT.START_TMS < SYSDATE() " +
        " AND FIRT.DATA_SRC_ID NOT IN ('MHI') AND FIRT.RTNG_CDE NOT IN ('WR','NR','WD','SD') " +
        " AND EXISTS (SELECT 1 FROM FT_T_RTQL RTQL WHERE RTQL.FINS_RTNG_OID = FIRT.FINS_RTNG_OID AND RTQL.END_TMS IS NULL AND RTQL.RTNG_QUAL_TYP = 'SOLICITATION STATUS' and RTQL.RTNG_QUAL_TXT = 'Solicited' ) " +
        " AND FIRT.INST_MNEM = :INST_MNEM_1<char[11]> AND BRM1.B3_RTNG_SET_OID = :B3_RTNG_SET_OID_2<char[11]> " +
        " AND COALESCE(BRM1.B3_RTNG_CURR_TYP ,'NULL') = :RTNG_CURR_TYP_3<char[20]> " +
        " AND (BRM1.RTNG_JURIS_NME = :RTNG_JURIS_NME_4<char[9]> OR (BRM1.RTNG_JURIS_NME IS NULL) )" +
        " ORDER BY VRTNG.RTNG_RANK_NUM, " +
        " (CASE WHEN FIRT.DATA_SRC_ID IN ('MOODYS_RDS','MOODYS RDS', 'MOODY') THEN 1  WHEN FIRT.DATA_SRC_ID IN ('SNPXF_RT','SAP') THEN 2  WHEN FIRT.DATA_SRC_ID IN ('FITCH') THEN 3 WHEN FIRT.DATA_SRC_ID IN ('BB', 'JCR') THEN 4  ELSE 5 END) ";

    // query for updating FT_TBLG1 with status of the currentling in progress Basel3 rating
    private String blg1Query = "INSERT INTO FT_T_BLG1 " +
        "	(BLG1_OID, B3_RTNG_SET_OID, INST_MNEM, STATUS_TYP, ADDL_INFO_TXT, TRACE_PATH, TRN_ID, CREATED_TMS,LAST_CHG_TMS,LAST_CHG_USR_ID) " +
        "	VALUES (NEW_OID(), :B3_RTNG_SET_OID_2<char[11]> , :INST_MNEM_3<char[11]>, :STATUS_TYP_4<char[40]>, :ADDL_INFO_TXT_5<char[1000]>, :TRACE_PATH_6<char[4000]>, :TRN_ID_7<char[17]>, SYSDATE(), SYSDATE(), 'CJavaMHIBasel3IssuerRatingDerivation')";

    private String existingDeriveRatingQuery = "SELECT FT_T_FIRT.RTNG_CDE, FT_T_FIRT.DATA_SRC_ID,FT_T_FIRT.FINS_RTNG_OID, FT_T_FIRT.RTNG_VALUE_OID FROM FT_T_FIRT " +
        "WHERE (FT_T_FIRT.END_TMS IS NULL OR FT_T_FIRT.END_TMS > SYSDATE()) " +
        "AND (FT_T_FIRT.SYS_EFF_END_TMS  IS NULL OR FT_T_FIRT.SYS_EFF_END_TMS > SYSDATE()) AND FT_T_FIRT.START_TMS < SYSDATE() " +
        "AND FT_T_FIRT.INST_MNEM =:INST_MNEM_1<char[11]> " +
        "AND FT_T_FIRT.RTNG_SET_OID =:RTNG_SET_OID_2<char[11]> " +
        "AND COALESCE(FT_T_FIRT.RTNG_CURR_TYP ,'NULL') = :RTNG_CURR_TYP_3<char[20]> " +
        "AND FT_T_FIRT.RTNG_PURP_TYP = :RTNG_PURP_TYP_4<char[9]> ";

    private String ratingSetsQuery = "SELECT DISTINCT B3_RTNG_SET_OID||'<DELM>'||COALESCE(B3_RTNG_CURR_TYP,'') FROM FT_T_BRM1 WHERE (RTNG_SET_TBL_TYP IS NULL OR RTNG_SET_TBL_TYP IN ('FINS','FIIS','ALL') ) AND END_TMS IS NULL " +
        " AND (RTNG_JURIS_NME IS NULL OR RTNG_JURIS_NME =:RTNG_JURIS_NME_1<char[8]>)";

    private String fetchCntrlGovCQSQry = "WITH QRY AS ( " +
        " SELECT FINS.INST_MNEM, FFRL.PRNT_INST_MNEM, (SELECT RTNG_SET_OID FROM FT_T_RTNG WHERE RTNG_SET_MNEM = 'B3ISRDRT' AND END_TMS IS NULL)B3ISSRDR " +
        " FROM FT_T_FINS FINS LEFT JOIN (SELECT * FROM FT_T_FFRL WHERE END_TMS IS NULL AND REL_TYP='CENTRALGOVTINST'  ) FFRL ON FFRL.INST_MNEM = FINS.INST_MNEM  " +
        " WHERE FINS.INST_MNEM  = :INST_MNEM<char[11]> ) " +
        " SELECT " +
    ...
    public boolean initialize(String[] parameters) {
        ...
        logger.info("Initialized CJavaMHIBasel3IssuerRatingDerivation Rule with Juristictions: " + Juristictions);
        return true;
    }
    ...
    public boolean process(XMLMessage msg, DatabaseObjectContainer dboc, ProcessorContext pContext, DatabaseAccess dbConnection, NotificationCreator notificationCreator) throws GSException {
        try {
            ...
            logger.info("Processing Java rule CJavaMHIBasel3IssuerRatingDerivation Rule");

            String msgClassification = msg.getStringField("MSG_CLASSIFICATION", new SegmentId(0));
            if (!"B3FINSRTNGCALBTCH".equalsIgnoreCase(msgClassification)) {
            ...
            String instMnem = msg.getStringField("INST_MNEM", firstSeg);
            String transactionID = pContext.getScopedTransaction().getTransactionID();
            logger.info("Processing Rule for INST_MNEM:" + instMnem + " and TrnId:" + transactionID);

            if (instMnem == null || instMnem.equals("")) {
                logger.info("No inst_mnem found. Exiting");
                return true;
            }
            ...
            for (String thisJuristiction: Juristictions) {
                ...
                for (String thisB3RatingSetCurr: B3RatingSetsCurr) {
                    ...
                    CJavaMHIBasel3RatingAttributes thisExistingDerivedRating = getExistingB3FinsRating(instMnem, thisB3RtngSetOid, thisB3RtngCurr, thisJuristiction, dbConnection);

                    // Get attributes for new Rating
                    CJavaMHIBasel3RatingAttributes newDerivedRating = getDerivedRatingAttributes(instMnem, thisB3RtngSetOid, thisB3RtngCurr, dbConnection, thisJuristiction);
                    ...
                    if (!newDerivedRating.isEmpty()) {
                        ...
                        if (!thisExistingDerivedRating.ratingCode.equals(newDerivedRating.ratingCode)) {
                            ...
                            String hqLqFlag = fetchRtngQltyByCntrlGovtCQSNum(instMnem, newDerivedRating.ratingCqsNum, thisB3RtngCurr, thisJuristiction, dbConnection, tracePathSB, addlInfoSB);

                            addAmendFinsRating(dbConnection, msg, instMnem, thisB3RtngSetOid, newDerivedRating, thisJuristiction, thisExistingDerivedRating, thisB3RtngCurr, hqLqFlag);
                            tracePathSB.append("->Storing in DB");
                            addUpdateBlg1Log(thisB3RtngSetOid, instMnem, statusTyp, tracePathSB.toString(), addlInfoSB.toString(), transactionID, dbConnection);
                        } else {
                        ...
                        if (!thisExistingDerivedRating.isEmpty()) {
                            ...
                            endDateExistingDerivedRating(dbConnection, msg, instMnem, thisExistingDerivedRating.ratingOid);
    ...
    CJavaMHIBasel3RatingAttributes getExistingB3FinsRating(String instMnem, String b3RtngSetOid, String b3RatingSetCurrTyp, String juristiction, DatabaseAccess dbConnection) throws Exception {
        logger.info("Within method getExistingFinsRating()");
        String ratingCode = "";
        String ratingVendor = "";
        String ratingOid = "";
    ...
    CJavaMHIBasel3RatingAttributes getDerivedRatingAttributes(String instMnem, String rtngB3SetOid, String b3RtngCurrType, DatabaseAccess dbConnection, String juristiction) throws Exception {
        ...
        if (derivedRating.isEmpty()) {
            ...
            logger.debug("getDerivedRatingAttributes -> " + derivedRating.getAttributesString());
        }
        return derivedRating;
    }
    void endDateExistingDerivedRating(DatabaseAccess dbConnection, XMLMessage msg, String instMnem, String ratingOid) {
        logger.info("Within method EndDateExistingDerivedRating end-date rating with FINS_RTNG_OID = '" + ratingOid + "'");
        ...
        SegmentId newSegmentId = msg.addSegment(XMLMessage.A_D_UPDATE, "FinancialInstitutionRating");
        msg.addField("INST_MNEM", newSegmentId, instMnem);
        msg.addField("DATA_SRC_ID", newSegmentId, "MHI");
        msg.addField("FINS_RTNG_OID", newSegmentId, ratingOid);
        msg.addField("SYS_EFF_END_TMS", newSegmentId, today);
        msg.addField("END_TMS", newSegmentId, today);

        logger.info("exiting method EndDateExistingDerivedRating()");
    }
    void addAmendFinsRating(DatabaseAccess dbConnection, XMLMessage msg, String instMnem, String b3RtngSetOid, CJavaMHIBasel3RatingAttributes newDerivedRating, String rtngJuristiction,
        CJavaMHIBasel3RatingAttributes thisExistingDerivedRating, String b3RtngCurrTyp, String hqLqFlag) throws Exception {
        logger.info("in method addAmendFinsRating()");
        ...
        msg.setSegmentAttribute(newSegmentId, "MATCH", "FIRT_MATCH_MHIB3");
        ...
        logger.info("exiting method addAmendFinsRating()");
    }
    private String fetchRtngQltyByCntrlGovtCQSNum(String instMnem, String ratingCqsNum, String b3RtngCurrTyp, String rtngJuristiction, DatabaseAccess dbConnection, StringBuilder tracePathSB, StringBuilder addlInfoSB) throws Exception {
        String hqLqFlag = "";
        ...
        logger.debug("Fetched values for InstMnem=" + instMnem + " and rtngJuristiction=" + rtngJuristiction + " : issrDflt=" + issrDflt + ", issrDfltLcl=" + issrDfltLcl + ", issrDfltFgn=" + issrDfltFgn + " and fnlExpCls=" + fnlExpCls + " Hence derived Value is fnlGovtCQS=" + fnlGovtCQS);
