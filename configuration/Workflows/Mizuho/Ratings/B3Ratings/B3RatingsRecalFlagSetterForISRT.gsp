...
USING (SELECT DISTINCT FT_T_ISSR.FINS_INST_MNEM FROM FT_T_ISSR
        INNER JOIN FT_T_ISSU ON FT_T_ISSR.INSTR_ISSR_ID = FT_T_ISSU.INSTR_ISSR_ID
        INNER JOIN FT_T_ISRT on FT_T_ISSU.INSTR_ID = FT_T_ISRT.INSTR_ID
        WHERE FT_T_ISRT.LAST_CHG_TMS &gt; ?
        AND (FT_T_ISSR.END_TMS IS NULL OR FT_T_ISSR.END_TMS &gt; now())
        AND FT_T_ISSU.INSTR_ID IN
        ...
        and FT_T_ISGP.PRNT_ISS_GRP_OID in (SELECT FT_T_ISGR.ISS_GRP_OID FROM FT_T_ISGR where FT_T_ISGR.END_TMS IS NULL and FT_T_ISGR.GRP_PURP_TYP = 'INTEREST')
        )
        AND EXISTS (SELECT 1 FROM FT_T_BRM1 WHERE FT_T_BRM1.VND_RTNG_SET_OID = FT_T_ISRT.RTNG_SET_OID AND FT_T_BRM1.END_TMS IS NULL 
AND (FT_T_BRM1.RTNG_SET_TBL_TYP IS NULL OR FT_T_BRM1.RTNG_SET_TBL_TYP != 'FINS'))) E
...
WHEN MATCHED AND STAT_CHAR_VAL_TXT != 'Y' THEN
  UPDATE set STAT_CHAR_VAL_TXT = 'Y', LAST_CHG_TMS = now(), LAST_CHG_USR_ID = 'B3IssuRating' 
WHEN NOT MATCHED THEN
  INSERT ( STAT_ID,STAT_DEF_ID,INST_MNEM,START_TMS,LAST_CHG_TMS,LAST_CHG_USR_ID,STAT_CHAR_VAL_TXT)
  VALUES (NEW_OID(),'B3RECALF',e.FINS_INST_MNEM,now(),now(),'B3IssuRating','Y')</objectValue>
<type>CONSTANT</type>
</item>
</parameters>
...
<lastUpdate id="33">2025-07-16T15:10:10.000+0100</lastUpdate>
<name id="34">B3RatingsRecalFlagSetterForISRT</name>
