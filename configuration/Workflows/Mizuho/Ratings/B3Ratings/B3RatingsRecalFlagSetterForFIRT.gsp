...
<clusteredCall>false</clusteredCall>
<directJoin>false</directJoin>
<name id="9">Set B3RECALF for FIRT</name>
...
USING (SELECT DISTINCT FT_T_FIRT.INST_MNEM FROM FT_T_FIRT WHERE FT_T_FIRT.LAST_CHG_TMS &gt; ?
AND FT_T_FIRT.INST_MNEM IN (SELECT FT_T_ISSR.FINS_INST_MNEM FROM FT_T_ISSR WHERE (FT_T_ISSR.END_TMS IS NULL OR FT_T_ISSR.END_TMS &gt; now()) AND
FT_T_ISSR.INSTR_ISSR_ID IN ( SELECT FT_T_IRGP.INSTR_ISSR_ID FROM FT_T_IRGP WHERE (FT_T_IRGP.END_TMS IS NULL OR FT_T_IRGP.END_TMS &gt; now())
AND FT_T_IRGP.PRT_PURP_TYP = 'INTEREST'
AND FT_T_IRGP.PRNT_ISSR_GRP_OID IN (SELECT FT_T_IRGR.ISSR_GRP_OID FROM FT_T_IRGR WHERE FT_T_IRGR.END_TMS IS NULL AND FT_T_IRGR.GRP_PURP_TYP = 'INTEREST')))
AND EXISTS (SELECT 1 FROM FT_T_BRM1 WHERE FT_T_BRM1.VND_RTNG_SET_OID = FT_T_FIRT.RTNG_SET_OID AND FT_T_BRM1.END_TMS IS NULL 
AND (FT_T_BRM1.RTNG_SET_TBL_TYP IS NULL OR FT_T_BRM1.RTNG_SET_TBL_TYP != 'ISSU'))) E
  ON (E.INST_MNEM = S.INST_MNEM AND S.STAT_DEF_ID = 'B3RECALF' AND (S.END_TMS IS NULL OR S.END_TMS &gt; now()))
WHEN matched and STAT_CHAR_VAL_TXT != 'Y' THEN
  UPDATE set STAT_CHAR_VAL_TXT = 'Y', LAST_CHG_TMS = now(), LAST_CHG_USR_ID = 'B3FinsRating' 
WHEN NOT MATCHED THEN
  INSERT ( STAT_ID,STAT_DEF_ID,INST_MNEM,START_TMS,LAST_CHG_TMS,LAST_CHG_USR_ID,STAT_CHAR_VAL_TXT)
  VALUES (new_oid(),'B3RECALF',e.INST_MNEM,now(),now(),'B3FinsRating','Y')</objectValue>
<type>CONSTANT</type>
</item>
</parameters>
...
<lastUpdate id="33">2025-07-16T15:00:29.000+0100</lastUpdate>
<name id="34">B3RatingsRecalFlagSetterForFIRT</name>
