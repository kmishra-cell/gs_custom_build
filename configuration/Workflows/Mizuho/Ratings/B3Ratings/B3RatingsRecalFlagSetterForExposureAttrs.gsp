...
USING (SELECT DISTINCT FT_T_FICL.INST_MNEM FROM FT_T_FICL WHERE FT_T_FICL.LAST_CHG_TMS &gt;  ?
      AND FT_T_FICL.INDUS_CL_SET_ID IN ('B3INIEXPCL', 'B3PRFNEXCL', 'B3EBFNEXCL')
) E
...
WHEN matched and STAT_CHAR_VAL_TXT != 'Y' THEN
  UPDATE set STAT_CHAR_VAL_TXT = 'Y', LAST_CHG_TMS = now(), LAST_CHG_USR_ID = 'B3ExpAttrs'
     
WHEN NOT MATCHED THEN
  INSERT ( STAT_ID,STAT_DEF_ID,INST_MNEM,START_TMS,LAST_CHG_TMS,LAST_CHG_USR_ID,STAT_CHAR_VAL_TXT)
  VALUES (NEW_OID(), 'B3RECALF',e.INST_MNEM, now(),now(),'B3ExpAttrs','Y')</stringValue>
<type>CONSTANT</type>
</item>
</parameters>
...
<lastUpdate id="33">2025-07-16T14:42:09.000+0100</lastUpdate>
<name id="34">B3RatingsRecalFlagSetterForExposureAttrs</name>
